...
// Calculate total commands automatically
...
void exec_command(char* input) {
    ...
    for (int i = 0; i < num_commands; i++) {
        if (strcmp(input, shell_commands[i].name) == 0) {
            shell_commands[i].handler(); // Call the function pointer
            print_prompt();
            return;
        }
    }
...
// Actual functions (at the end cuz of bloat)
void cmd_help() {
    kprint("Supernova Fallback Shell - Available Commands:\n", -1, 0x0E);
    for (int i = 0; i < num_commands; i++) {
        kprint("  ", -1, 0x07);
        kprint(shell_commands[i].name, -1, 0x0B); // Print name in Cyan
        kprint(" - ", -1, 0x07);
        kprint(shell_commands[i].description, -1, 0x07); // Print description
        putchar('\n', 0x07);
    }
}
